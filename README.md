...
2.  **Enable Services**:
    ...
    - Go to **Build > Storage** and create a storage bucket.
3.  **Get Firebase Config**:
    - In your Firebase project, go to **Project settings** (the gear icon).
    - Under the "General" tab, scroll down to "Your apps".
    - Click the web icon (`</>`) to create a new web app.
    - After registering the app, you will be given a `firebaseConfig` object.
4.  **Configure the App**:
    - Copy the `firebaseConfig` object.
    - In this project, open `src/firebase/config.ts`.
    - Replace the placeholder configuration with the one you copied from your Firebase project.

### Installation & Running the App

1.  **Clone the repository**:
    ```bash
    git clone https://github.com/your-username/medicare-assist.git
    cd medicare-assist
    ```
...
Open [http://localhost:3000](http://localhost:3000) with your browser to see the result. You can start by creating an account on the register page.
